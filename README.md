...
이 프로젝트는 기본적으로 로컬 환경에서의 사용을 가정하고 있습니다. 만약 이 애플리케이션을 실제 운영 환경에 배포할 계획이라면, 다음 보안 사항을 반드시 검토하고 적용해 주세요:

- **API 키 관리**: `.env` 파일이 `git` 관리 대상에 포함되지 않도록 `.gitignore`에 추가되었는지 확인하세요. 절대 API 키를 버전 관리 시스템(GitHub 등)에 올리지 마십시오.
- **입력값 검증**: 사용자 입력값이 프롬프트에 직접 포함되므로, 프롬프트 인젝션(Prompt Injection) 공격의 위험이 있습니다. 사용자 입력에 대한 필터링과 검증을 강화하는 것을 권장합니다.
- **에러 처리**: 운영 환경에서는 `st.exception(e)`를 사용하지 마십시오. 시스템 내부 정보가 사용자에게 노출될 수 있으므로, 서버 측 로깅 시스템을 사용하여 에러 상세 정보를 기록하십시오.
- **파일 업로드**: 단순히 파일 확장자만 확인하는 것보다, 파일의 실제 내용을 검증하여 안전한 파일만 처리하도록 구현하는 것이 좋습니다.
